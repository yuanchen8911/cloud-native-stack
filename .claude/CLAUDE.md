...
2. **Add templates** only if the bundler needs custom manifests:
...
func (b *Bundler) Make(ctx context.Context, input *result.RecipeResult, 
    ...
    if err := b.WriteValuesFile(filePath, values); err != nil {
        return nil, err
    }
...
**Note on templates:**
Most bundlers don't need a templates directory - they only generate values.yaml and checksums.txt. Templates are only needed for bundlers that generate custom Kubernetes manifests (e.g., GPU Operator's DCGM exporter ConfigMap).

For bundlers with custom manifests, create a templates directory:
```
