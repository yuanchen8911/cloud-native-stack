# NVIDIA Cloud Native Stack v14.1 - Install Guide for Developers
## Introduction
...
NVIDIA Cloud Native Stack v14.1 includes:
- Ubuntu 22.04 LTS
- Containerd 2.0.3
...
```
wget https://github.com/containerd/containerd/releases/download/v2.0.3/cri-containerd-cni-2.0.3-linux-amd64.tar.gz
```

```
sudo tar --no-overwrite-dir -C / -xzf cri-containerd-cni-2.0.3-linux-amd64.tar.gz
```

```
rm -rf cri-containerd-cni-2.0.3-linux-amd64.tar.gz
```
...
```
wget https://github.com/containerd/containerd/releases/download/v2.0.3/cri-containerd-cni-2.0.3-linux-arm64.tar.gz
```

```
sudo tar --no-overwrite-dir -C / -xzf cri-containerd-cni-2.0.3-linux-arm64.tar.gz
```

```
rm -rf cri-containerd-cni-2.0.3-linux-arm64.tar.gz
```
...
```
```
 sudo apt install -y -q kubelet=1.31.6-1.1  kubectl=1.31.6-1.1  kubeadm=1.31.6-1.1 
```
```
...
```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=/run/containerd/containerd.sock --kubernetes-version="v1.31.6"
```

Eecute the following command for `CRI-O` systems:

```
sudo kubeadm init --pod-network-cidr=192.168.32.0/22 --cri-socket=unix:/run/crio/crio.sock --kubernetes-version="v1.31.6"
```
...
#yourhost        Ready    control-plane,master   10m   v1.31.6
```
...
NAME             STATUS   ROLES                  AGE   VERSION
#yourhost        Ready    control-plane,master   10m   v1.31.6
#yourhost-worker Ready                           10m   v1.31.6
```
